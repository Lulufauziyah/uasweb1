...
<?php
	...
	$qjan = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Jan' && `year` = '$year'");
	$fjan = $qjan->fetch_array();
	$qfeb = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Feb' && `year` = '$year'");
	$ffeb = $qfeb->fetch_array();
	$qmar = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Mar' && `year` = '$year'");
	$fmar = $qmar->fetch_array();
	$qapr = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Apr' && `year` = '$year'");
	$fapr = $qapr->fetch_array();
	$qmay = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'May' && `year` = '$year'");
	$fmay = $qmay->fetch_array();
	$qjun = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Jun' && `year` = '$year'");
	$fjun = $qjun->fetch_array();
	$qjul = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Jul' && `year` = '$year'");
	$fjul = $qjul->fetch_array();
	$qaug = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Aug' && `year` = '$year'");
	$faug = $qaug->fetch_array();
	$qsep = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Sept' && `year` = '$year'");
	$fsep = $qsep->fetch_array();
	$qoct = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Oct' && `year` = '$year'");
	$foct = $qoct->fetch_array();
	$qnov = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Nov' && `year` = '$year'");
	$fnov = $qnov->fetch_array();
	$qdec = $conn->query("SELECT COUNT(*) as total FROM `radiological` WHERE `month` = 'Dec' && `year` = '$year'");
	...
	<div id = "content">
		...
		<div id = "target">
			...
			<table id = "table" cellspacing = "0" class = "display">
				...
				<?php 
					$conn = new mysqli("localhost", "root", "", "hcpms");
					$query = $conn->query("SELECT * FROM `radiological` NATURAL JOIN `itr` GROUP BY `itr_no` ORDER BY `rad_id` DESC");
				...
				?>
					<tr>
						...
						<td><center><a class = "btn btn-primary" href = "xray_record.php?itr_no=<?php echo $fetch['itr_no']?>"><span class = "glyphicon glyphicon-search"></span> All Records</a></center></td>
					</tr>
				<?php
					}
				?>		
