...
<body>
	...
	<div id = "content">
		...
		<?php
			...
			$f1 = $q1->fetch_array();
		?>
		<div class = "alert alert-info">Sputum Record / <?php echo $f1['firstname']." ".substr($f1['middlename'], 0, 1).". ".$f1['lastname']?><a class = "btn btn-success" style = "float:right; margin-top:-7px;" href = "sputum.php"><span class = "glyphicon glyphicon-hand-right"></span> Back</a></div>
		<table id = "table" cellspacing = "0" class = "display">
			...
			<?php 
				$conn = new mysqli("localhost", "root", "", "hcpms");
				$q = $conn->query("SELECT * FROM `sputum` WHERE `itr_no` = '$_GET[itr_no]' ORDER BY `date_of_examination` DESC");
