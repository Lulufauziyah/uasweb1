...
<?php
	...
	$qjan = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Jan' && `year` = '$year'");
	$fjan = $qjan->fetch_array();
	$qfeb = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Feb' && `year` = '$year'");
	$ffeb = $qfeb->fetch_array();
	$qmar = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Mar' && `year` = '$year'");
	$fmar = $qmar->fetch_array();
	$qapr = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Apr' && `year` = '$year'");
	$fapr = $qapr->fetch_array();
	$qmay = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'May' && `year` = '$year'");
	$fmay = $qmay->fetch_array();
	$qjun = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Jun' && `year` = '$year'");
	$fjun = $qjun->fetch_array();
	$qjul = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Jul' && `year` = '$year'");
	$fjul = $qjul->fetch_array();
	$qaug = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Aug' && `year` = '$year'");
	$faug = $qaug->fetch_array();
	$qsep = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Sept' && `year` = '$year'");
	$fsep = $qsep->fetch_array();
	$qoct = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Oct' && `year` = '$year'");
	$foct = $qoct->fetch_array();
	$qnov = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Nov' && `year` = '$year'");
	$fnov = $qnov->fetch_array();
	$qdec = $conn->query("SELECT COUNT(*) as total FROM `fecalisys` WHERE `month` = 'Dec' && `year` = '$year'");
...
<body>
	...
	<div id = "content">
		...
		<br />
		<div id = "target">
			<div class = "alert alert-info">Fecalysis Record</div>
			<table id = "table" cellspacing = "0" class = "display">
				...
				<?php 
					$conn = new mysqli("localhost", "root", "", "hcpms");
					$query = $conn->query("SELECT * FROM `fecalisys` NATURAL JOIN `itr` GROUP BY `itr_no` ORDER BY `fecalisys_id` DESC");
				...
				?>
					<tr>
						...
						<td><center><a class = "btn btn-primary" href = "fecalysis_record.php?itr_no=<?php echo $fetch['itr_no']?>"><span class = "glyphicon glyphicon-search"></span> All Records</a></center></td>
					</tr>
				<?php
					}
				?>		
