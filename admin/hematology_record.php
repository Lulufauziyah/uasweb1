...
<body>
	...
	<div id = "content">
		...
		<?php
			...
			$f1 = $q1->fetch_array();
		?>
		<div class = "alert alert-info">Hematology Record / <?php echo $f1['firstname']." ".substr($f1['middlename'], 0, 1).". ".$f1['lastname']?><a class = "btn btn-success" style = "float:right; margin-top:-7px;" href = "hematology.php"><span class = "glyphicon glyphicon-hand-right"></span> Back</a></div>
		<table id = "table" cellspacing = "0" class = "display">
			...
			<?php 
				$conn = new mysqli("localhost", "root", "", "hcpms");
				$q = $conn->query("SELECT * FROM `hematology` NATURAL JOIN `itr` WHERE `itr_no` = '$_GET[itr_no]' ORDER BY `date_requested` DESC");
			...
			?>
				<tr>
					...
					<td><center><a class = "btn btn-info" href = "hematology_form.php?itr_no=<?php echo $f['itr_no']?>&hem_id=<?php echo $f['hem_id']?>"><span class = "glyphicon glyphicon-search"></span> View Detail</a><center></td>
				</tr>
			<?php
				}
			?>	
