...
<body>
	...
	<div id = "content">
		...
		<?php
			...
			$f1 = $q1->fetch_array();
		?>
		<div class = "alert alert-info">Maternity Record / <?php echo $f1['firstname']." ".substr($f1['middlename'], 0, 1).". ".$f1['lastname']?><a class = "btn btn-success" style = "float:right; margin-top:-7px;" href = "maternity.php"><span class = "glyphicon glyphicon-hand-right"></span> Back</a></div>
		<table id = "table" cellspacing = "0" class = "display">
			...
			<tbody>
			<?php
				$q2 = $conn->query("SELECT * FROM `itr` WHERE `itr_no` = '$_GET[itr_no]'");
				$f2 = $q2->fetch_array();
				$q = $conn->query("SELECT * FROM `birthing` WHERE `itr_no` = '$f2[itr_no]'");
			...
			?>
				<tr>
					...
					<td><center><a class = "btn btn-info" href = "maternity_form.php?itr_no=<?php echo $f['itr_no']?>&birth_id=<?php echo $f['birth_id']?>"><span class = "glyphicon glyphicon-search"></span> View Detail</a><center></td>
				</tr>
			<?php
				}
			?>	
