...
<body>
	...
	<div id = "content">
		...
		<?php
			$conn = new mysqli("localhost", "root", "", "hcpms");
			$q1 = $conn->query("SELECT * FROM `itr` WHERE `itr_no` = '$_REQUEST[itr_no]'");
			$f1 = $q1->fetch_array();
		?>
		<div class = "alert alert-info">Prenatal Record / <?php echo $f1['firstname']." ".substr($f1['middlename'], 0, 1).". ".$f1['lastname']?><a class = "btn btn-success" style = "float:right; margin-top:-7px;" href = "maternity.php"><span class = "glyphicon glyphicon-hand-right"></span> Back</a></div>
		<table id = "table" cellspacing = "0" class = "display">
			...
			<tbody>
			<?php 
				$a = $conn->query("SELECT * FROM `prenatal` WHERE `itr_no` = '$_GET[itr_no]'");
				while($b = $a->fetch_array()){
			?>
				<tr>
					<td><?php echo date("m/d/Y", strtotime($b['date']))?></td>
					<td><?php echo $b['attending_physician']?></td>
					<td><?php echo $b['radiologist']?></td>
					<td><center><a class = "btn btn-info" href = "prenatal_form.php?itr_no=<?php echo $b['itr_no']?>&prenatal_no=<?php echo $b['prenatal_no']?>"><span class = "glyphicon glyphicon-search"></span> View Detail</a><center></td>
				</tr>
			<?php
				}
			?>	
