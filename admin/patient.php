		...
		<div class = "panel panel-primary">
			...
			<div class = "panel-body">
				...
				<table id = "table" class = "display" width = "100%" cellspacing = "0">
					...
					<?php
						$conn = new mysqli("localhost", "root", "", "hcpms");
						$query = $conn->query("SELECT * FROM `itr` ORDER BY `itr_no` DESC");
						while($fetch = $query->fetch_array()){
						$id = $fetch['itr_no'];
						$q = $conn->query("SELECT COUNT(*) as total FROM `complaints` where `itr_no` = '$id' && `status` = 'Pending'");
						$f = $q->fetch_array();
					?>
						<tr>
							...
							<a onclick="confirmationDelete(this);return false;" href = "delete_patient.php?id=<?php echo $fetch['itr_no']?>" class = "btn btn-sm btn-danger"><i class = "glyphicon glyphicon-remove"></i> Delete</a></center></td>
						</tr>
					<?php
						}
						$conn->close();
