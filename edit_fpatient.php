...
<html lang = "en">
	...
	<div class = "well">
		...
		<div id = "p_fdental" class = "panel panel-success">	
			...
			<?php
				...
				$_GET['lastname'];
				$conn = new mysqli("localhost", "root", "", "hcpms");
				$query = $conn->query("SELECT * FROM `itr` WHERE `itr_no` = '$_GET[id]' && `lastname` = '$_GET[lastname]'");
				$fetch = $query->fetch_array();
			?>
			<div class = "panel-body">
				<form id = "form_dental" action = "edit_patient.php?id=<?php echo $fetch['itr_no']?>&lastname=<?php echo $fetch['lastname']?>" method = "POST" enctype = "multipart/form-data">
					<div style = "float:right;" class = "form-inline">
						...
						<input class = "form-control" value = "<?php echo $fetch['family_no']?>" size = "3" type = "text" name = "family_no">
					</div>
					<br />
					<br />
					<br />
					...
					<div class = "form-inline">
						...
						<input class = "form-control" size = "3" value = "<?php echo $fetch['age']?>"  name = "age" type = "text">
					...
					<br />
					<div class = "form-inline">
						<button style = "float:right;" class = "btn btn-warning" name = "edit_fpatient"><span class = "glyphicon glyphicon-pencil"></span> EDIT</button>
					</div>
				</form>
			</div>	
		</div>	
	...
	<div id = "footer">
		<label class = "footer-title">&copy; Copyright Patient Health Center Management System 2024</label>
	</div>
	</body>
