<?php
	if(ISSET($_POST['save_d'])){
		...
		$conn->query("INSERT INTO `dental` VALUES('', '$date', '$dentist', '$tooth', '$itr_no', '$user_id', '$month', '$year')");
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Dental' && `com_id` = '$_GET[comp_id]'");
		header("location:view_dental_record.php");
		$conn->close();
	}
	if(ISSET($_POST['save_f'])){
		$q = $conn->query("SELECT * FROM `complaints` WHERE `com_id` = '$_GET[comp_id]'");
		$f = $q->fetch_array();
		...
		$conn = new mysqli("localhost", 'root', '', 'hcpms');
		$conn->query("INSERT INTO `fecalisys` VALUES('', '$date_of_request', '$requested_by', '$color', '$consistency', '$pus_cells', '$rbc', '$fat_globules', '$occult_blood', '$others_chem', '$ova', '$larva', '$adult_forms', '$cyst', '$trophozoites', '$others_pro', '$remarks', '$date_reported', '$pathologist', '$medical_technologist', '$itr_no', '$user_id', '$month', '$year')");
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Fecalysis' && `com_id` = '$_GET[comp_id]'");
	...
	if(ISSET($_POST['save_h'])){
		...
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Hematology' && `com_id` = '$_GET[comp_id]'");
	...
	if(ISSET($_POST['save_u'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", 'root', '', 'hcpms');
		$conn->query("INSERT INTO `urinalysis` VALUES('', '$date_of_request', '$color', '$transparency', '$specific_gravity', '$ph', '$sugar', '$protein', '$pregnacy_test', '$pus_cells', '$rbc', '$cast', '$urates', '$uric_acid', '$cal_ox', '$epith_cells', '$mucus_threads', '$others', '$pathologist', '$medical_technologist', '$itr_no', '$user_id', '$month', '$year')");
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Urinalysis' && `com_id` = '$_GET[comp_id]'");
	...
	if(ISSET($_POST['save_r'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", 'root', '', 'hcpms');
		$conn->query("INSERT INTO `rehabilitation` VALUES('', '$diagnosis', '$type_of_disability', '$subjective', '$objective', '$assessment', '$plan', '$date', '$itr_no', '$user_id', '$month', '$year')");
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Rehabilitation' && `com_id` = '$_GET[comp_id]'");
	...
	if(ISSET($_POST['save_s'])){
		...
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Sputum' && `com_id` = '$_GET[comp_id]'");
	...
	if(ISSET($_POST['save_x'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", 'root', '', 'hcpms');
		$conn->query("INSERT INTO `radiological` VALUES('', '$case_no', '$referred_by', '$clinical_impression', '$room_bed_no', '$type_of_examination', '$date_taken', '$radiologist', '$itr_no', '$user_id', '$month', '$year')");
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Xray' && `com_id` = '$_GET[comp_id]'");
	...
	if(ISSET($_POST['save_p'])){
		...
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Prenatal' && `com_id` = '$_GET[comp_id]'");
	...
	if(ISSET($_POST['save_patient'])){
		...
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[id]' && `section` = 'Maternity' && `com_id` = '$_GET[comp_id]'");
	...
	if(ISSET($_POST['save_birth'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", "root", "", "hcpms");
		$conn->query("INSERT INTO `birthing` VALUES('', '$date', '$time', '$chief_complaint', '$history', '$lmp', '$edc', '$aog', '$g', '$p', '$a', '$b', '$c', '$d', '$bp1', '$bp2', '$pr', '$rr', '$t', '$head_neck', '$chest', '$heart', '$abdomen', '$fhb', '$loc', '$extremities', '$vulva', '$vagina', '$cervix', '$uterus', '$bow', '$presentation', '$vaginal_discharge', '$staff', '$itr_no', '$user_id', '$month', '$year')");
		$conn->query("UPDATE `complaints` SET `status` = 'Done' WHERE `itr_no` = '$_GET[itr_no]' && `section` = 'Maternity' && `com_id` = '$_GET[comp_id]'");
