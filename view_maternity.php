...
<html lang = "en">
	...
	<div class = "well">
		...
		<div class = "panel panel-success">	
			<div class = "panel-heading">
			<?php
				$q = $conn->query("SELECT * FROM `itr` WHERE `itr_no` = '$_GET[itr_no]'");
				$f = $q->fetch_array();
				$q1 = $conn->query("SELECT COUNT(*) as total FROM `complaints` where `status` = 'Pending' && `itr_no` = '$f[itr_no]' && `section` = 'Prenatal'");
				$f1 = $q1->fetch_array();
				$q2 = $conn->query("SELECT COUNT(*) as total1 FROM `complaints` where `status` = 'Pending' && `itr_no` = '$f[itr_no]' && `section` = 'Maternity'");
				$f2 = $q2->fetch_array();
			?>
				<label>Patient Information / <label class = "text-warning"><?php echo $f['firstname']." ".substr($f['middlename'], 0,1).". ".$f['lastname']?></label></label>
				<a style = "float:right; margin-top:-4px;" href = "maternity.php" class = "btn btn-info"><span class = "glyphicon glyphicon-hand-right"></span> BACK</a>
				<a style = "float:right; margin-top:-4px; margin-right:5px;" href = "birthing_pending.php?itr_no=<?php echo $f['itr_no']?>" class = "btn btn-info">REQUEST BIRTHING ADMISSION <span class = "badge"> <?php echo $f2['total1']?></span></a>
				<a style = "float:right; margin-top:-4px; margin-right:5px;" href = "prenatal_pending.php?itr_no=<?php echo $f['itr_no']?>" class = "btn btn-info">REQUEST PRENATAL <span class = "badge"> <?php echo $f1['total']?></span></a>
