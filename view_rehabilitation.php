	...
	<div class = "well">
		...
		<div class = "panel panel-success">	
			...
			<?php
				...
				$f = $q->fetch_array();
				$q1 = $conn->query("SELECT COUNT(*) as total FROM `complaints` where `status` = 'Pending' && `itr_no` = '$f[itr_no]' && `section` = 'Rehabilitation'");
				$f1 = $q1->fetch_array();
			?>
				<label>Patient Information / <label class = "text-warning"><?php echo $f['firstname']." ".substr($f['middlename'], 0,1).". ".$f['lastname']?></label></label>
				<a style = "float:right; margin-top:-4px;" href = "rehabilitation.php" class = "btn btn-info"><span class = "glyphicon glyphicon-hand-right"></span> BACK</a>
				<a style = "float:right; margin-top:-4px; margin-right:5px;" href = "rehabilitation_pending.php?itr_no=<?php echo $f['itr_no']?>" class = "btn btn-info">REQUEST <span class = "badge"> <?php echo $f1['total']?></span></a>
